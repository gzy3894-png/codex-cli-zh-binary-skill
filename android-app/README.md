...
2.1.3 起浏览器桥对单个坏请求做异常隔离，队列请求先完成 legacy request 再暴露给 App 消费；页面加载会等待 userscript 注入回调后再向终端返回 `open` 完成。userscript 注入使用延迟重试、回调解析和本地 `userscripts.log`，注入失败会反馈给 `codex-browser`，避免 `open -> get-text` 抢跑或静默失败。
