...
2.5.14 是 2.5.13 CI 编译失败后的正式前滚热修。2.5.13 tag 已推送但未产出可安装正式 APK；请安装 2.5.14 或更新版本。

## 修复内容

- 修复 `MainActivity` 文件托盘/文本托盘提交路径中的 Kotlin nullable 编译错误，正式 GitHub Actions APK 构建可继续完成。
...
- Codex 历史扫描保留 0/2/5/10/20/40/60/90 秒有限启动重扫，打开侧栏也会即时刷新，覆盖迁移完成后的 UUIDv7 rollout 会进入 registry。
...
- 在 Grok worker 里退出后再输入 `claude` 不应把原 Grok 对话改名为 Claude；新 Claude 应从启动台创建独立 worker。

## 版本

- `versionName=2.5.14`
- `versionCode=80`
- runtime epoch：`apk-2.5.14`
