...
formats the patched Rust source with the repository toolchain, verifies
`cargo fmt --all -- --check`, runs `git diff --check`, and exits before Zig,
Perl, or Cargo build setup.
