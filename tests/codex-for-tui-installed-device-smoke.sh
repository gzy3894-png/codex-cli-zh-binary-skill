...
if [ -s "$app_codex_home/config.toml" ]; then
  ...
  if [ "${CODEX_TUI_EXPECTED_VERSION_CODE:-}" = "87" ]; then
...
if [ "${CODEX_TUI_EXPECTED_VERSION_CODE:-}" = "87" ]; then
  ...
  safe_grep "runtime_epoch=apk-2.5.21" "$build_key_file" ||
...
if [ -z "$workspace_release" ] &&
  [ "${CODEX_TUI_EXPECTED_VERSION_CODE:-}" = "87" ]
then
...
if [ "${CODEX_TUI_EXPECTED_VERSION_CODE:-}" = "87" ]; then
  current_workspace_report="$app_codex_home/install-state/apk-upgrades/2.5.21/workspace-migration.json"
  [ -s "$current_workspace_report" ] ||
    fail "2.5.21 workspace migration report missing: $current_workspace_report"
  safe_grep '"already_migrated": true' "$current_workspace_report" ||
    safe_grep '"ok": true' "$current_workspace_report" ||
    fail "2.5.21 workspace migration report is not successful"
