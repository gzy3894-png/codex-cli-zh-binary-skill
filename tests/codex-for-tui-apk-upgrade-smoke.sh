...
EOF
  ...
  support_archive="codex-support-2.5.17.tgz"
...
migration = json.loads(
    (home / "install-state/apk-upgrades/2.5.17/workspace-migration.json").read_text(
        encoding="utf-8"
    )
)
...
launch = json.loads(
    (home / "install-state/apk-upgrades/2.5.17/launch-check.json").read_text(encoding="utf-8")
)
assert launch["runtime_home"] == str(runtime)
assert "apk-2.5.17" in launch["sqlite_home"]
...
test_failure_rolls_back_managed_and_config() {
  ...
  [ ! -e "$home/install-state/apk-upgrades/2.5.17/complete" ] ||
...
assert not (runtime / "state_5.sqlite").exists()
assert (home / "state_5.sqlite").is_file()
corrupt = home / "install-state/apk-upgrades/2.5.17/corrupt"
...
test_missing_binary_is_installed() {
  ...
  assert_contains "$home/install-state/apk-upgrades/2.5.17/complete" "version_code=83"
}
...
test_stale_empty_lock_is_recovered() {
  ...
  [ -s "$home/install-state/apk-upgrades/2.5.17/complete" ] ||
