...
EOF
  ...
  support_archive="codex-support-2.5.4.tgz"
...
launch = json.loads(
    (home / "install-state/apk-upgrades/2.5.4/launch-check.json").read_text(encoding="utf-8")
)
...
test_failure_rolls_back_managed_and_config() {
  ...
  [ ! -e "$home/install-state/apk-upgrades/2.5.4/complete" ] ||
...
test_missing_binary_is_installed() {
  ...
  assert_contains "$home/install-state/apk-upgrades/2.5.4/complete" "version_code=70"
}
...
test_stale_empty_lock_is_recovered() {
  ...
  [ -s "$home/install-state/apk-upgrades/2.5.4/complete" ] ||
