...
EOF
  ...
  support_archive="codex-support-2.5.18.tgz"
...
migration = json.loads(
    (home / "install-state/apk-upgrades/2.5.18/workspace-migration.json").read_text(
        encoding="utf-8"
    )
)
...
launch = json.loads(
    (home / "install-state/apk-upgrades/2.5.18/launch-check.json").read_text(encoding="utf-8")
)
assert launch["runtime_home"] == str(runtime)
assert "apk-2.5.18" in launch["sqlite_home"]
...
test_failure_rolls_back_managed_and_config() {
  ...
  [ ! -e "$home/install-state/apk-upgrades/2.5.18/complete" ] ||
...
# 2.5.5+: sessions/history are shared via control-home symlinks.
assert (runtime / "sessions").is_symlink()
...
assert not (runtime / "state_5.sqlite").exists()
assert (home / "state_5.sqlite").is_file()
corrupt = home / "install-state/apk-upgrades/2.5.18/corrupt"
...
PY
  ...
  # 2.5.5+: profile launch already shares sessions/history via control-home
...
# 2.5.17+: runtime sessions always symlink to control home (shared list), never
# to the legacy profile tree. Fixture mixed.jsonl lacks session UUID so it stays
# under config-profiles until a proper rollout import; shared symlink must still
# work and must not point at the empty/wrong legacy path.
assert (mixed_runtime / "sessions").is_symlink()
assert (mixed_runtime / "sessions").resolve() == (home / "sessions").resolve()
assert (home / "config-profiles/mixed-v1/sessions/2026/07/10/mixed.jsonl").is_file()
...
test_missing_binary_is_installed() {
  ...
  assert_contains "$home/install-state/apk-upgrades/2.5.18/complete" "version_code=84"
}
...
test_stale_empty_lock_is_recovered() {
  ...
  [ -s "$home/install-state/apk-upgrades/2.5.18/complete" ] ||
