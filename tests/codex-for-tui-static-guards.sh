...
test_apk_upgrade_guards() {
  ...
  assert_file_contains "$APK_UPGRADER" 'RELEASE="2.4.7"'
  assert_file_contains "$APK_UPGRADER" 'VERSION_CODE="62"'
  ...
  assert_file_contains "$APK_PAYLOAD_PREPARE" 'RELEASE="2.4.7"'
  assert_file_contains "$APK_PAYLOAD_PREPARE" 'VERSION_CODE="62"'
  ...
  assert_file_contains "$APP_BUILD_GRADLE" 'release"] != "2.4.7"'
  assert_file_contains "$APP_BUILD_GRADLE" 'version_code"] != "62"'
  ...
  assert_file_contains "$CODEX_COMMON" ': "${CODEX_ZH_RUNTIME_EPOCH:=apk-2.4.7}"'
  assert_file_contains "$SCRIPT_DIR/lib/codex-zh-local.sh" 'epoch="${CODEX_ZH_RUNTIME_EPOCH:-apk-2.4.7}"'
...
test_debug_build_uses_test_package_name() {
  ...
  assert_file_contains "$APP_BUILD_GRADLE" 'versionCode = 62'
  assert_file_contains "$APP_BUILD_GRADLE" 'versionName = "2.4.7"'
}
...
test_release_workflow_signature_gate() {
  ...
  assert_file_contains "$BUILD_WORKFLOW" 'CODEX_TUI_EXPECTED_VERSION_CODE: "62"'
  assert_file_contains "$BUILD_WORKFLOW" 'CODEX_TUI_EXPECTED_VERSION_NAME: 2.4.7'
  ...
  assert_file_contains "$BUILD_WORKFLOW" 'body_path: docs/codex-for-tui-2.4.7-release-notes.md'
...
EOF
  ...
  printf '%s\n' "$output" | grep -F 'apk-2.4.7' >/dev/null 2>&1 ||
