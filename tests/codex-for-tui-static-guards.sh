...
test_apk_upgrade_guards() {
  ...
  assert_file_contains "$APK_UPGRADER" 'RELEASE="2.4.6"'
  assert_file_contains "$APK_UPGRADER" 'VERSION_CODE="61"'
  ...
  assert_file_contains "$APK_PAYLOAD_PREPARE" 'RELEASE="2.4.6"'
  assert_file_contains "$APK_PAYLOAD_PREPARE" 'VERSION_CODE="61"'
  ...
  # 2.4.6: multi-line ready guide with command results
  assert_file_contains "$BOOTSTRAP" '可输入命令'
  assert_file_contains "$BOOTSTRAP" '启动 Codex TUI'
  assert_file_contains "$BOOTSTRAP" '管理站点、模型与压缩策略'
  assert_file_contains "$BOOTSTRAP_ASSET" '可输入命令'
  ...
  # 2.4.6: interactive shell + tty hygiene for live echo
  ...
  assert_file_contains "$APP_BUILD_GRADLE" 'release"] != "2.4.6"'
  assert_file_contains "$APP_BUILD_GRADLE" 'version_code"] != "61"'
  ...
  assert_file_contains "$CODEX_COMMON" ': "${CODEX_ZH_RUNTIME_EPOCH:=apk-2.4.6}"'
  assert_file_contains "$SCRIPT_DIR/lib/codex-zh-local.sh" 'epoch="${CODEX_ZH_RUNTIME_EPOCH:-apk-2.4.6}"'
...
test_debug_build_uses_test_package_name() {
  ...
  assert_file_contains "$APP_BUILD_GRADLE" 'versionCode = 61'
  assert_file_contains "$APP_BUILD_GRADLE" 'versionName = "2.4.6"'
}
...
test_release_workflow_signature_gate() {
  ...
  assert_file_contains "$BUILD_WORKFLOW" 'CODEX_TUI_EXPECTED_VERSION_CODE: "61"'
  assert_file_contains "$BUILD_WORKFLOW" 'CODEX_TUI_EXPECTED_VERSION_NAME: 2.4.6'
  ...
  assert_file_contains "$BUILD_WORKFLOW" 'body_path: docs/codex-for-tui-2.4.6-release-notes.md'
...
test_image_preview_bridge_asset() {
  ...
  assert_file_contains "$MEDIA_PREVIEW_PANE" 'SendPreviewDialog'
  assert_file_contains "$MEDIA_PREVIEW_PANE" '附加说明（可选）'
  assert_file_contains "$MEDIA_PREVIEW_PANE" 'sendTarget = preview'
  assert_file_contains "$MEDIA_PREVIEW_PANE" 'onSendToAi(preview, message)'
...
EOF
  ...
  printf '%s\n' "$output" | grep -F 'apk-2.4.6' >/dev/null 2>&1 ||
