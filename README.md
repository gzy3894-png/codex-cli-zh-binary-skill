...
[![Release](https://img.shields.io/badge/release-v2.2.2-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v2.2.2)
...
2.2.2 是终端流畅度优化版：App 侧把终端输出刷新合并到屏幕帧，不再让每次文本变化都立刻触发重绘；Compose 托盘、浏览器状态、会话折叠和桥接轮询也做了去重/降频，减少长输出时的卡顿和 UI 抖动。新增 `$PREFIX/local/perf/terminal.status` 作为轻量排障状态文件，便于 Agent 判断合帧和 bridge 工作状态。
...
已经安装 2.0.x/2.1.x/2.2.0/2.2.1 的用户需要从 Releases 下载并覆盖安装 2.2.2 APK。覆盖安装后重新打开终端，新会话会自动同步 `codex-panel`、`codex-preview`、`codex-browser`、`codex-session`、`codex-rtk`、`codex-context` 等 APK 内置桥接命令。
