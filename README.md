...
2.5.28 为第三方 Responses API 增加明确的模型能力兜底：无法匹配官方能力目录的模型默认使用 `272000` token 上下文、`217600` token（80%）自动压缩阈值，以及 `low/medium/high/xhigh` 四档推理强度（默认 `medium`）。已识别模型继续保留官方上下文与额外的 `max/ultra` 档位。新建或改选第三方模型后会提示用户确认/输入上下文长度，并按输入值自动计算 80% 阈值；覆盖值会在刷新模型目录和 APK 升级重建后保留。发布说明见 `docs/codex-for-tui-2.5.28-release-notes.md`。
...
2.5.26 修复 `/model` 只改隔离 runtime、`~/.codex/config.toml` 要等下次启动才更新的问题：交互启动时由 generation 约束的轻量 watcher 即时同步当前 profile 与 control config。固定压缩策略现在会覆盖模型 `comp_hash` 的无条件切模压缩；在远低于固定阈值时切换模型不再连续 auto compact。`codex --version/--help` 也改为只读直通，不再物化 runtime 或消费待写默认值。发布说明见 `docs/codex-for-tui-2.5.26-release-notes.md`。

## 重要：2.5.20 启动性能热修（大会话 dual-sync）

2.5.20 修的是用户体感「Codex 启动将近半分钟」：会话库变大后，provider dual-sync 在 marker 失效时会整文件扫描 rollout（本机约 93 个文件 / 173MB，最大约 94MB）只为改首行 `session_meta`。本版改为首行读写 + 同长 provider 原地补丁；warm path 跳过已链接 runtime 的重复 seed/import，并在 V2 热启动跳过多余 `status` 引擎进程。2.5.19 的配置模式统一格式与 krill `service_tier` 修复继续保留。发布说明见 `docs/codex-for-tui-2.5.20-release-notes.md`。

## 重要：2.5.19 配置模式 / 启动 fast path / 本机清理热修

2.5.19 在 2.5.18 之上统一配置格式：中转站只切换模型策略、API 与 Key；上下文长度、压缩策略、权限等为全站通用项。顺带修复 krill 无效 `service_tier=high`、APK 升级反复导入 `-legacy` 站、以及 provider 同步 fast path。发布说明见 `docs/codex-for-tui-2.5.19-release-notes.md`。

## 重要：2.5.18 原生 /resume provider 双写热修

2.5.18 在 2.5.17 之上修复「SQLite restamp 被启动 backfill 冲掉」：Codex 会从 rollout `session_meta.model_provider` 重建索引。本版按 Codex++ 方式在启动前同时改写共享 `session_meta` 与各 runtime SQLite，跨中转原生 `/resume` 不再空白。发布说明见 `docs/codex-for-tui-2.5.18-release-notes.md`。

## 重要：2.5.0 会话隔离（基于 2.4.10）

2.5.0 交付会话隔离 P0：抽屉/顶栏显示带固定前缀的会话名（`codex-` / `claude-` / `shell-`），支持新建时选 agent、长按重命名、进程重启后按注册表恢复标签列表；resume 只认绑定的 UUID，改显示名不会破坏恢复。独立模块不改全局 `CODEX_HOME` 与 proot 路径，2.4.10 引导回 shell 与 2.4.2+ 离线升级路径保持。发布说明见 `docs/codex-for-tui-2.5.0-release-notes.md`。

## 重要：2.4.10 引导后自动回 shell（基于 2.4.9）

2.4.10 修复 2.4.9「引导文字输出完后卡住、不出现 prompt，只能 Ctrl+C 才到 `localhost:/root#`」：根因是 `init-host` 按行过滤 guest stderr，busybox ash 的无换行 PS1 / CSI `6n` 被永久卡住。现改为交互 shell 前把 stderr 绑回会话 PTY，并把 proot 过滤器改为字节/前缀感知。2.4.9 的启动即时文案与 managed 脚本 stamp，以及 2.4.8 的 `$HOME` 纯 shell / 扩大 proot 过滤继续保留。发布说明见 `docs/codex-for-tui-2.4.10-release-notes.md`。
...
- 2.5.17：原生 `/resume` provider 过滤热修；仅 restamp SQLite（会被 backfill 冲掉，已被 2.5.18 取代）。
- 2.5.16：跨中转/配置档统一会话列表；runtime `sessions` 深合并到 control home 并共享链接；启动修复全部 idle runtime；legacy 导入不再指向空 sessions。
...
- 2.5.11：修复历史恢复 `init-host` 执行拒绝与窗口 ID 复用导致的旧 UUID 串绑。
...
- 2.4.10 热修：引导结束后自动出现 shell prompt（无需 Ctrl+C）；字节级 proot stderr 过滤，避免卡住 ash PS1。
- 2.4.9 热修：启动立刻有文案；按 version stamp 跳过每会话重写大型 managed 脚本（含 rtk）。
- 2.4.8 热修：引导后 shell 固定 `$HOME`；扩大 proot 噪声过滤；稳定 `PROOT_TMP_DIR`。
- 2.4.7 热修：引导结束后纯 shell 交接；文件托盘多选 + 顶部发送；临时文件短名 `图片N`/`视频N`/`文本N`。
- 2.4.6 热修：多行就绪引导；交互 shell + 默认输入模式修正，输入过程实时回显。
- 2.4.5 体验：默认 shell、设置可恢复自动进 Codex、过滤 proot 噪声、Codex launcher 使用 workspace 减黄字、日常升级静默秒退。
...
4. 选择 Codex 配置方式：
   ...
   - 第三方 Responses API：输入 Base URL 和 API Key，脚本会请求 `/models`，再让你选择默认模型、上下文长度和推理等级。
...
第三方模型目录刷新也可以手动执行。Provider `/models` 只决定可用模型 ID，真实推理等级、上下文窗口和工具能力优先从随包 OpenAI Codex 官方目录精确合并；未知模型兜底为 `272000` token 上下文、80% 自动压缩阈值和 `low/medium/high/xhigh` 四档推理（默认 `medium`）。只有明确识别的模型才增加官方声明的 `max/ultra` 等额外档位。新建或改选模型时可以确认/输入上下文长度，按模型保存的覆盖值在手动刷新和 APK 升级后继续保留。所有 `codex-auto-*` 辅助模型保留在目录中但标记为隐藏，使 `/model` 首层直接进入完整普通模型页；刷新会保留当前 `model` 和 `model_reasoning_effort`，如果当前选择已不再受支持会回落到该模型的上游默认值并报告：
