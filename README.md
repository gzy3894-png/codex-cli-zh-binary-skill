...
[![Release](https://img.shields.io/badge/release-v2.5.8-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v2.5.8)
...
## 重要：2.5.8 对话恢复与配置修复（基于 2.5.6）

2.5.8 在 2.5.6 配置物化修复之上，侧栏升级为 Codex/Claude **对话管理器**：自动导入已有 JSONL 历史、持久化真实 UUID，选择对话时复用或新建 PTY 并执行 resume。关闭终端窗口只结束 PTY，不删除对话；对话操作只归档。配置仍采用 `common ⊕ managed ⊕ runtime-local`，删除 control 通用字段不会被旧 runtime 复活。发布说明见 `docs/codex-for-tui-2.5.8-release-notes.md`。
...
已经安装 2.0.x～2.5.6 的用户，直接从 Releases 下载并覆盖安装 2.5.8 APK，然后打开 App。首次启动会自动完成二进制、launcher、脚本、配置档 V2、模型目录和 SQLite 运行世代升级；之后默认进入 shell（引导结束后 cwd 为统一 workspace `/root/workspace`，无需 Ctrl+C），输入 `codex` 再启动。对话抽屉会导入 Codex/Claude 历史并按 UUID 恢复，关闭窗口不会删除对话。无需再执行任何 Codex 更新或修复命令。
