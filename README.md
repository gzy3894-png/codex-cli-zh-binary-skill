...
[![Release](https://img.shields.io/badge/release-v2.5.16-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v2.5.16)
...
## 重要：2.5.16 跨中转统一会话列表热修

2.5.16 保证无论切换哪套中转/配置档，原生 Codex `/resume`（不依赖 `--all`）都能看到同一完整会话列表：把各 runtime 私有或错误的 `sessions` 深合并进 control home 并统一为共享符号链接，启动时修复全部 idle runtime，纠正 legacy 导入把 sessions 指到空目录的问题。发布说明见 `docs/codex-for-tui-2.5.16-release-notes.md`。
...
已经安装 2.0.x～2.5.13 的用户，直接从 Releases 下载并覆盖安装 2.5.16 APK，然后打开 App。首次启动会自动完成二进制、launcher、脚本、配置档 V2、模型目录、旧 Codex rollout/SQLite 工作区迁移和运行世代升级；之后进入固定启动台。输入 `codex`、`claude` 或 `grok` 会创建独立工作窗口；历史对话按真实 UUID 恢复，关闭窗口不会删除对话。无需再执行任何 Codex 更新或修复命令。
