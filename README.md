...
[![Release](https://img.shields.io/badge/release-v2.4.3-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v2.4.3)
...
## 重要：2.4.3 热修（基于 2.4.2 完整环境升级）

2.4.3 修复 2.4.2 真机回归：`gpt-5.6-sol/terra/luna` 不再使用 musl 上不可用的 `code_mode_only`，工具调用恢复为普通 shell 路径；配置引擎会在目录重建时持续规范化该字段。会话仍按 profile 隔离，历史对话在对应配置 runtime 中，可用 `codex resume --all` 或切回原配置恢复。发布说明见 `docs/codex-for-tui-2.4.3-release-notes.md`。
...
发布安全说明：正式 APK 构建必须使用 GitHub Secrets/离线签名输入，仓库内不再提供正式 keystore fallback；Release 门禁会校验包名、版本号、非 debuggable、正式签名指纹和 APK 内离线 Codex 载荷。Android 不支持普通覆盖安装降级，2.4.3（`versionCode=58`）出现问题时只发布更高 `versionCode` 的前滚修复包。
...
已经安装 2.0.x、2.1.x、2.2.x、2.3.x、2.4.0、2.4.1 或 2.4.2 的用户，直接从 Releases 下载并覆盖安装 2.4.3 APK，然后打开 App。首次启动会自动完成二进制、launcher、脚本、配置档 V2、模型目录和 SQLite 运行世代升级；无需再执行任何 Codex 更新或修复命令。
