...
[![Release](https://img.shields.io/badge/release-v2.4.7-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v2.4.7)
...
## 重要：2.4.7 纯 shell 交接与文件托盘多选（基于 2.4.6）

2.4.7 在多行就绪引导后干净退出到纯交互 shell（加强 `stty`/`ENV` 回显交接），并重做文件托盘：多选勾选、顶部统一发送、显示名改为 `图片N`/`视频N`/`文本N`。2.4.6 的输入模式映射与 2.4.5 默认 shell / proot 过滤 / workspace / 静默升级继续保留；2.4.3 `tool_mode` 修复继续保留。发布说明见 `docs/codex-for-tui-2.4.7-release-notes.md`。
...
发布安全说明：正式 APK 构建必须使用 GitHub Secrets/离线签名输入，仓库内不再提供正式 keystore fallback；Release 门禁会校验包名、版本号、非 debuggable、正式签名指纹和 APK 内离线 Codex 载荷。Android 不支持普通覆盖安装降级，2.4.7（`versionCode=62`）出现问题时只发布更高 `versionCode` 的前滚修复包。
...
已经安装 2.0.x、2.1.x、2.2.x、2.3.x、2.4.0、2.4.1、2.4.2、2.4.3、2.4.4、2.4.5 或 2.4.6 的用户，直接从 Releases 下载并覆盖安装 2.4.7 APK，然后打开 App。首次启动会自动完成二进制、launcher、脚本、配置档 V2、模型目录和 SQLite 运行世代升级；之后默认进入 shell（引导结束后为纯交互 shell），输入 `codex` 再启动。无需再执行任何 Codex 更新或修复命令。
