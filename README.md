...
[![Release](https://img.shields.io/badge/release-v1.0.0-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v1.0.0)
...
6. 选择 Codex 配置方式：
   ...
   - 第三方 Responses API：输入 Base URL 和 API Key，脚本会请求 `/models`，再让你选择启用模型和默认模型。
...
安装完成后，重新打开 App 会自动进入 Codex。也可以在终端里手动运行：
...
- 第三方 API 不只是填一个 Key，还要处理 Base URL、`/models`、模型列表和默认模型。
- 配置失败后很容易不知道该重新执行哪一步。
