...
[![Release](https://img.shields.io/badge/release-v2.2.3-blue)](https://github.com/gzy3894-png/codex-cli-zh-binary-skill/releases/tag/codex-for-tui-v2.2.3)
...
2.2.3 是 2.2.2 的真机门禁热修版：保留终端输出合帧、Compose 状态去重和 bridge 低频兜底轮询，同时修复打开 Custom Tabs/外部浏览器后 Activity 进入后台导致文件托盘、浏览器和会话折叠 bridge 停止消费的问题。新增 `$PREFIX/local/perf/terminal.status` 作为轻量排障状态文件，便于 Agent 判断合帧和 bridge 工作状态。
...
已经安装 2.0.x/2.1.x/2.2.x 的用户需要从 Releases 下载并覆盖安装 2.2.3 APK。覆盖安装后重新打开终端，新会话会自动同步 `codex-panel`、`codex-preview`、`codex-browser`、`codex-session`、`codex-rtk`、`codex-context` 等 APK 内置桥接命令。
...
本仓库使用 GitHub Actions 构建 release APK。构建前会先运行安装器 smoke test，覆盖 bootstrap 不自动联网、显式脚本更新、本地启动器不 preflight、模型目录显式刷新等路径。
